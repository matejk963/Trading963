...
- ~~**Screener filter-control gaps:** UI lacks `eps_growth` / `rev_growth` / `eps_accel_filter` controls + `sort_by` missing `mcap`.~~ **DONE 2026-06-12** — full filter+sort parity with the original screener: added the 3 growth/accel dropdowns, the 3 RS-Chg (1W/1M/3M) range filters + their active-filter tags, and the `mcap` sort option. Also **implemented the RS-momentum backend** the RS-Chg filters depend on (`ComputedStore.rs_rank_changes` — `rs_chg1w/1m/3m` diffed from `classification_history.rs_rank` at 5/21/63 trading-day lags; the stored `rs_rank` shares the original's 6-mo-return percentile basis so the deltas are exact). Without it those filters were dead (always 0 rows). See log REPORT 2026-06-12.

...
Status: Build complete; post-build screener parity work done (2026-06-13) — filter/sort parity, RS-momentum backend, 2 wrong-answer bug fixes, Sectors stats-table + Map view restored, column picker on both tables (12/13 slices; 2b GPU deferred). 314 tests pass / 6 red from a data incident. See the **2026-06-13 HANDOFF** in log.md for current state + the open data decision.
...
- ⚠️ **DATA INCIDENT — needs a decision (blocks 6 red tests):** the price panels `data/mktt/{close,high,low,volume}.parquet` were corrupted (2026-06-13 ~09:08, truncated write when the dev server was killed mid price-update) and restored from `*.parquet.bak_0609` (Jun-11 snapshot). That snapshot is older than the kernel **golden fixtures'** baseline (now unrecoverable) → 6 `test_kernel` parity reds. DECIDE: (A) re-fetch latest prices then re-baseline `generate_golden.py`, or (B) accept restored data + re-baseline only. Delete `*.parquet.corrupt_0613` when satisfied. See 2026-06-13 HANDOFF.
- ~~**Screener filter-control gaps** + sector/Map parity.~~ **DONE 2026-06-12/13** — full filter+sort parity (growth/accel dropdowns, RS-Chg 1W/1M/3M filters+tags, `mcap` sort) + RS-momentum backend (`rs_rank_changes`); two wrong-answer bugs fixed (stage presets never filtered; sector medians over full universe not passed set) incl. `stage_transitions` for `trans12`; **Sectors view** restored to the original rich stats table; **column picker** drives both tables; **Map view** (sector × dimension composition) reimplemented on the new pipeline. See log REPORTs 2026-06-12/13.

...
## 2026-06-13 · orchestrator · HANDOFF (current state — supersedes the 2026-06-12 pick-up HANDOFF)
**Read order for a fresh agent:** this entry → `plan-dev.md` (status + Open items) → the REPORTs above from 2026-06-12 (filter parity onward). Design spec = `refactor/TARGET_ARCHITECTURE.md`; as-built = `docs/product/mktt-architecture.md`; decisions = `docs/adr/0001,0002`.

**STATE — branch `chore/agents-workflow-port` @ `b94e38e` (pushed to origin).**
- **Tests: 314 passing / 6 failing.** The 6 reds are `test_kernel::test_parity_kernel_vs_stage_classifier[*]` — NOT a code defect; they stem from the data incident below (restored an older price snapshot than the golden fixtures' baseline). Everything else (screener/store/sections/kernel-non-parity/GEX) is green.
- **Run:** `cd src/mktt && MKTT_PG_DSN=postgresql://postgres:postgres@10.123.0.9:5432/etc_db python -c "import app; app.create_app().run(host='127.0.0.1', port=<FREE>, debug=False, use_reloader=False, threaded=True)"`. A server is currently up on **46293**. Port 5001 collides with the energy app — always pick a free port. **Do NOT `kill` the server mid price-update** (that caused the incident below).

**SHIPPED since the 2026-06-12 pick-up HANDOFF (each has a REPORT above):**
1. **Screener filter + sort parity** (`7380730`→`1b8e0b1`): added the missing `eps_growth`/`rev_growth`/`eps_accel_filter` dropdowns, the `rschg1w/1m/3m` range filters + active tags, the `mcap` sort. Built the **RS-momentum backend** `ComputedStore.rs_rank_changes` (rs_chg from `classification_history` rs_rank diffed at 5/21/63 trading-day lags) — the rschg filters were previously dead (always 0 rows).
2. **Two wrong-answer bugs fixed** (`ad69bdc`): (a) stage presets never filtered (stage2≡stage4≡full set) → added the Stage_Class/Transition filter + `ComputedStore.stage_transitions` for `trans12`; (b) sector medians were over the full universe instead of the passed set. Both now match the original.
3. **Sector "Sectors" view restored to the original rich stats table** (`baed61a`): full per-sector median columns + By-Industry/All-Stocks tabs + per-stock rows; added `rs_chg*` to `_page_row`.
4. **Column picker now drives the sector table too** (`2b0419c`): union of both tables' columns, hides across the nested hierarchy, added the Debt group.
5. **"Map" view restored** (`b94e38e`): `service._sector_map_summary` — sector × dimension composition (pca_regime/stage/eps_momentum/eps_growth/rs_bucket/rs_momentum/pe_vs_sector) over the passed set, embedded for client-side dimension switching + the %Sector/%Category modes. Reimplemented on the new pipeline (the legacy `/api/sector_map` endpoint still reads the broken parquet path and is unused by the UI).

**OPEN — needs a user decision (blocking the 6 red tests):**
- **Data incident (2026-06-13 ~09:08):** the gitignored price panels `data/mktt/{close,high,low,volume}.parquet` were corrupted (truncated write when the dev server was killed mid price-update). Recovered by restoring `*.parquet.bak_0609` (Jun 11 snapshot, now auto-updated to 2026-06-12 by the healthy background updater); corrupt originals set aside as `*.parquet.corrupt_0613`. The restored snapshot is ~2 days older than the snapshot the kernel **golden fixtures** were baselined against (that snapshot is overwritten/unrecoverable) → 6 parity reds. **DECIDE:** (A) re-fetch latest prices via `data_manager` then re-baseline fixtures (`generate_golden.py`); or (B) accept the Jun-11/12 data and just re-run `generate_golden.py` to re-baseline. Also: delete the `*.corrupt_0613` files when satisfied.

**OPEN — pre-existing (unchanged, see plan-dev.md "Open items"):** systematic old↔new value-parity pass; full UI walkthrough of every section; deferrals (GPU 2b, legacy_bp→Monitor, old-module cleanup, NTM=FY1 proxy, asset-overlay coverage only SPY, thinner options/rrg/macro layouts).

**Data facts (for reference):** universe selection = US exchanges NMS/NYQ/ASE/NCM/NGM via `yf.screen` (`avgdailyvol3m > 1000`) then turnover (price × avg-3M-vol) ≥ $500k, sorted by market cap. Panels: 4835 close / 4891 high-low-volume symbols × 1542 days (2020-04-24 → 2026-06-12); DB MKCompStore = 4835 materialized.
